...
select distinct e.*, m.ename
from emp e
         left join emp m on e.mgr = m.empno
where e.job in ('职员', '销售员');
-- 15、查询员工姓名、职位、薪水、薪资等级
select e.ename, e.job, e.sal, s.grade
from emp e
         left join salgrade s on sal <= s.hisal and sal >= s.losal;
...
select product_id, product_name, c.name
from product
right join category c on c.id = product.category_id;